...
bool IsPowerOn = false;             // 电源使能：true-开启，false-关闭

uint16_t g_adc_raw_buf[128] = {0};  // ADC原始采样缓冲区（128个值，偶数列-电压，奇数列-电流）
float Voltage = 0;                  // 实时输出电压（单位：V）
float Current = 0;                  // 实时输出电流（单位：A）
...
/* ========================== 函数声明 ========================== */
// 绘制相关函数
static void DrawBasicElement(void); // 绘制界面基础元素
...
void CB_DPS_ADC_ConvCpltCallback(ADC_HandleTypeDef *hadc) {
    if (hadc->Instance == ADC1) {
        ...
        for (uint16_t i = 0; i < 64; i++) {
            sum_ch1 += g_adc_raw_buf[2 * i];
            sum_ch2 += g_adc_raw_buf[2 * i + 1];
        }

        // 计算均值并叠加校准偏移
        uint32_t temp1 = (sum_ch1 >> 6) + UserParam.DPS_Voltage_Original;
        uint32_t temp2 = (sum_ch2 >> 6) + UserParam.DPS_Current_Original;
        ...
        Current = (float) (temp2 > 4096 ? 0 : temp2) * 3.3f / 4096.0f * UserParam.DPS_Current_Factor
                  - UserParam.DPS_Current_Coefficient * Voltage - UserParam.DPS_Current_Constant;
...
void Resume_DpsCoreTask(void) {
    ...
    HAL_ADC_Start_DMA(&hadc1, (uint32_t *) g_adc_raw_buf, 128);
